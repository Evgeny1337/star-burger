...
- `YANDEX_GEOCODER_API_KEY` — секретный кюч для работы с сервисом Yandex Geocoder

## Быстрое обновление кода на сервере
Для деплоя новой версии кода на сервере выполните:
```bash
./deploy_star_burger
```
Данный файл лежит в папке `/opt/`
